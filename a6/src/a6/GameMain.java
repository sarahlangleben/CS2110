...
public class GameMain {
    ...
    private static void createAndShowGUI() {
        ...
        // verified by TA
        startButton.addActionListener((ActionEvent el) -> game.startGame());
        ...
        game.addPropertyChangeListener("GameScore",
            (PropertyChangeEvent es) -> scoreLabel.setText("Score: " + game.getScore()));
        ...
        // target radius to the slider's current value.
        // verified by TA
        sizeSlider.addChangeListener((ChangeEvent cl) -> game.setTargetRadius(sizeSlider.getValue()));
        ...
        speedSlider.addChangeListener(
            (ChangeEvent cl) -> game.setTargetTimeMillis(speedSlider.getValue()));
    ...
    private static void addSliderLabels(JSlider slider, String minLabel,
        ...
        // ta verified
        JLabel minl= new JLabel(minLabel);
        JLabel maxl= new JLabel(maxLabel);
        labels.put(slider.getMinimum(), minl);
        labels.put(slider.getMaximum(), maxl);
    ...
    private static void saveScore(JFrame frame, int score) {
        ...
        //File workingDirectory = new File(System.getProperty("user.dir")); //DELETE THIS WHEN YOU SUBMIT
        final JFileChooser fc = new JFileChooser(); //RESET TO DEFAULT WHEN YOU SUBMIT?
        fc.setDialogTitle("Save");
        int returnVal = fc.showSaveDialog(frame);
        if(returnVal == JFileChooser.APPROVE_OPTION){
            File fileToSave = fc.getSelectedFile();
            try(PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter(fileToSave, true)))){
                out.println(score);
            }
            catch(IOException e){
                JOptionPane.showMessageDialog(frame, e.toString(), e.getClass().getName(), JOptionPane.ERROR_MESSAGE);
            }
        }
